...
>>> addr = pybitcointools.pubtoaddr(pubkey)
>>> addr
...
field **sequence** is used in conjuction with the **locktime**
...
Since this previous transaction is recorded in the
[blockchain][bci_outpoint] the information in **outpoint** is enough
to determine which address we are moving funds from (`15CsXo...`), and
how many satoshis we want to transfer (3916000).

Since we are requesting to move funds from this address, we need to
verify that we are the rightful owner of this address and this is done
in the below **script** data field.
...
The **script** data field consists of two parts. The first part is the
**public key** of the destination address `15CsXo...` of the outpoint
transaction. The second part is created by taking the full transaction
data structure, setting these input script values to empty strings,
and then signing this truncated structure with the private key
corresponding to the destination address of the outpoint transaction.

Let's see if we can extract the public key from the script:

<pre>
'script': '493046022100b999de2e23127ec2edf16e2f267b4c2df57b9766059369cee85cbc0a41be6882022100d09c405f825eec986ca2bf6f35d1267ad7d595042fca4b4f7af3f9adfea68d330141<b>040bf69616981e5970c992a0762f441abcadfed9fc4630fa5e1b82ab00e81d16905d3820e073e1bd4a9dcfed336f4bf25edc634c2e174989767d299748359c2daf</b>',
</pre>

Take the highlighted string and compute the Hash160 of it followed by
a Base58Check encoding:
...
identified the public key in the script. The rest is the signed
message, together with some padding.
...
Furthermore, since the whole transaction is signed a malicious actor
